# ADR-113: Use local JSON policies for the executable reference and define conformance requirements for OPA or Cedar adapters
...
The tutorial stays dependency-light and provider-neutral. A production adapter may use OPA, Cedar or another engine only if it preserves deny-by-default, schema validation, immutable bundles, local enforcement, decision evidence, hard-control invariants and all AUTH-001/BR-001 boundaries.
