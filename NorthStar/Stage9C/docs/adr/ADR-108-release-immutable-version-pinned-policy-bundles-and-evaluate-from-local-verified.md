# ADR-108: Release immutable, version-pinned policy bundles and evaluate from local verified caches
...
Policy is validated, tested, independently approved, released with a digest and distributed to local PEP caches. Runs pin bundle ID/version/digest. This avoids a central per-request bottleneck while retaining provenance and rollback.
